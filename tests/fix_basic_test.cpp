...
int main() {
  ...
  try {
    ...
    std::cout << "   ✓ Interactive Brokers FIX support: "
              << (ibSupported ? "Yes" : "No") << std::endl;

    bool coinbaseSupported = factory.isFixSupported("coinbase_fix");
    std::cout << "   ✓ Coinbase FIX support: "
              << (coinbaseSupported ? "Yes" : "No") << std::endl;

    // Test 3: Configuration test
    std::cout << "\n2. Testing FIX configuration..." << std::endl;
    auto ibConfig = factory.getDefaultFixConfig(
        FixConnectorFactory::Exchange::INTERACTIVE_BROKERS);
    std::cout << "   ✓ IB Config - Host: " << ibConfig.host << ":"
              << ibConfig.port << std::endl;
    std::cout << "   ✓ IB Config - FIX Version: " << ibConfig.fixVersion
              << std::endl;
    std::cout << "   ✓ IB Config - Target: " << ibConfig.targetCompId
              << std::endl;
    ...
    bool credSet = credentials->setCredentials("interactive_brokers",
                                               "DU123456", "password");
    std::cout << "   ✓ Credentials set: " << (credSet ? "Success" : "Failed")
              << std::endl;

    auto apiKey = credentials->getApiKey("interactive_brokers");
    std::cout << "   ✓ API Key retrieved: " << (apiKey ? *apiKey : "Not found")
              << std::endl;

    // Test 5: Order creation
    std::cout << "\n4. Testing order creation..." << std::endl;
    pinnacle::Order testOrder("TEST123", "AAPL", pinnacle::OrderSide::BUY,
                              pinnacle::OrderType::LIMIT,
                              150.0, // price
                              100.0, // quantity
                              pinnacle::utils::TimeUtils::getCurrentNanos());

    std::cout << "   ✓ Order created - ID: " << testOrder.getOrderId()
              << std::endl;
    std::cout << "   ✓ Order details - " << testOrder.getSymbol() << " "
              << (testOrder.getSide() == pinnacle::OrderSide::BUY ? "BUY"
                                                                  : "SELL")
              << " " << testOrder.getQuantity() << "@" << testOrder.getPrice()
              << std::endl;
