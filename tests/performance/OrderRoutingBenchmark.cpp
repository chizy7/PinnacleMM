...
std::vector<MarketData> createTestMarketData() {
    return {
        {"Coinbase", 50000.0, 50100.0, 10.0, 8.0, utils::TimeUtils::getCurrentNanos(), 1000.0, 500.0, 0.001, 0.005},
        {"Binance", 49990.0, 50090.0, 15.0, 12.0, utils::TimeUtils::getCurrentNanos(), 1200.0, 600.0, 0.0008, 0.003},
        {"Kraken", 50010.0, 50110.0, 8.0, 6.0, utils::TimeUtils::getCurrentNanos(), 800.0, 400.0, 0.0015, 0.004},
        {"Gemini", 50005.0, 50105.0, 12.0, 9.0, utils::TimeUtils::getCurrentNanos(), 900.0, 450.0, 0.0012, 0.0035}
    };
}

// Helper function to create execution request
ExecutionRequest createTestExecutionRequest(const std::string& strategy, double quantity) {
    ExecutionRequest request;
    request.requestId = "BENCH_REQ_" + std::to_string(utils::TimeUtils::getCurrentNanos());
    request.order = Order(
        "BENCH_ORDER_" + std::to_string(utils::TimeUtils::getCurrentNanos()),
        "BTC-USD",
        OrderSide::BUY,
        OrderType::LIMIT,
        50000.0,
...
static void BM_TWAPStrategy_Planning(benchmark::State& state) {
    TWAPStrategy strategy(static_cast<int>(state.range(0)), std::chrono::seconds(30));
    auto marketData = createTestMarketData();
...
static void BM_OrderRouter_SubmitOrder(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        request.requestId = "BENCH_REQ_" + std::to_string(utils::TimeUtils::getCurrentNanos()) + "_" + std::to_string(state.iterations());
        request.order = Order(
            request.requestId,
            "BTC-USD",
            OrderSide::BUY,
...
static void BM_OrderRouter_MultipleStrategies(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        request.requestId = "MULTI_BENCH_" + std::to_string(utils::TimeUtils::getCurrentNanos()) + "_" + std::to_string(strategyIndex);
...
static void BM_OrderRouter_LargeOrderTWAP(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        auto request = createTestExecutionRequest("TWAP", static_cast<double>(state.range(0)));
        request.requestId = "LARGE_TWAP_" + std::to_string(utils::TimeUtils::getCurrentNanos());
...
static void BM_OrderRouter_MultiVenueVWAP(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        request.requestId = "MULTI_VWAP_" + std::to_string(utils::TimeUtils::getCurrentNanos());
...
BENCHMARK(BM_OrderRouter_ConcurrentSubmissions)->Arg(2)->Arg(4)->Arg(8)->Arg(16);
