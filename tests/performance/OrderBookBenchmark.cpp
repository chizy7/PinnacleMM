...
static void BM_OrderBook_ExecuteMarketOrder(benchmark::State& state) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        orderBook->addOrder(createOrder(bidOrderId, "BTC-USD", OrderSide::BUY, bidPrice, bidQuantity));
        
        // Add sell orders
        double askPrice = askPriceDist(rng);
        double askQuantity = quantityDist(rng);
        std::string askOrderId = "ask-" + std::to_string(i);
        orderBook->addOrder(createOrder(askOrderId, "BTC-USD", OrderSide::SELL, askPrice, askQuantity));
    }
...
static void BM_OrderBook_ConcurrentOperations(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (int t = 0; t < numThreads; ++t) {
            threads.emplace_back([&orderBook, &rng, &priceDist, &quantityDist, &sideDist, &threadCounter, t]() {
                int localCounter = threadCounter.fetch_add(1, std::memory_order_relaxed);
                ...
                for (int i = 0; i < 100; ++i) {
                    ...
                    if (op == 0) {
                        ...
                        double price = priceDist(rng);
                        double quantity = quantityDist(rng);
                        std::string orderId = "t" + std::to_string(t) + "-order-" + std::to_string(i);
                        orderBook->addOrder(createOrder(orderId, "BTC-USD", side, price, quantity));
                    } else if (op == 1) {
...
BENCHMARK(BM_LockFreeOrderBook_ConcurrentOperations)->Arg(2)->Arg(4)->Arg(8)->Arg(16);
