...
void testBestPriceStrategy() {
    ...
    Order testOrder("TEST_001", "BTC-USD", OrderSide::BUY, OrderType::LIMIT, 50000.0, 1.0, utils::TimeUtils::getCurrentNanos());
    ...
    std::vector<MarketData> marketData = {
        {"Coinbase", 50100.0, 50200.0, 5.0, 3.0, utils::TimeUtils::getCurrentNanos(), 1000.0, 500.0, 0.002, 0.005},
        {"Binance", 50050.0, 50150.0, 8.0, 6.0, utils::TimeUtils::getCurrentNanos(), 1200.0, 600.0, 0.001, 0.003},
        {"Kraken", 50080.0, 50180.0, 4.0, 2.0, utils::TimeUtils::getCurrentNanos(), 800.0, 400.0, 0.0015, 0.004}
    };
    
    auto results = strategy.planExecution(request, marketData);
    
    assert(results.size() == 1);
    assert(results[0].targetVenue == "Binance"); // Should select Binance (lowest ask + fees)
    
    std::cout << "✓ BestPriceStrategy selected venue: " << results[0].targetVenue << std::endl;
}

void testTWAPStrategy() {
    std::cout << "Testing TWAPStrategy..." << std::endl;
    
    TWAPStrategy strategy(5, std::chrono::seconds(10)); // 5 slices, 10 seconds apart
    
    Order testOrder("TEST_002", "ETH-USD", OrderSide::SELL, OrderType::LIMIT, 3000.0, 10.0, utils::TimeUtils::getCurrentNanos());
    ExecutionRequest request;
    request.requestId = "REQ_002";
    request.order = std::move(testOrder);
    
    std::vector<MarketData> marketData = {
        {"Coinbase", 2990.0, 3010.0, 20.0, 15.0, utils::TimeUtils::getCurrentNanos(), 500.0, 250.0, 0.001, 0.005}
    };
    ...
    assert(std::abs(totalQuantity - 10.0) < 0.001); // Total should equal original quantity
    
    std::cout << "✓ TWAPStrategy created " << results.size() << " slices, total quantity: " << totalQuantity << std::endl;
}
...
void testVWAPStrategy() {
    ...
    Order testOrder("TEST_003", "BTC-USD", OrderSide::BUY, OrderType::LIMIT, 50000.0, 5.0, utils::TimeUtils::getCurrentNanos());
    ExecutionRequest request;
    request.requestId = "REQ_003";
    request.order = std::move(testOrder);
    
    std::vector<MarketData> marketData = {
        {"Venue1", 50000.0, 50100.0, 10.0, 8.0, utils::TimeUtils::getCurrentNanos(), 1000.0, 500.0, 0.001, 0.003},
        {"Venue2", 50010.0, 50110.0, 12.0, 10.0, utils::TimeUtils::getCurrentNanos(), 2000.0, 1000.0, 0.0015, 0.004},
        {"Venue3", 50020.0, 50120.0, 8.0, 6.0, utils::TimeUtils::getCurrentNanos(), 500.0, 250.0, 0.002, 0.005}
    };
    ...
    for (const auto& result : results) {
        ...
        std::cout << "  Venue: " << result.targetVenue << ", Quantity: " << result.order.getQuantity() << std::endl;
    }
    
    std::cout << "✓ VWAPStrategy allocated total quantity: " << totalAllocated << std::endl;
}
...
void testMarketImpactStrategy() {
    ...
    Order testOrder("TEST_004", "ETH-USD", OrderSide::SELL, OrderType::LIMIT, 3000.0, 20.0, utils::TimeUtils::getCurrentNanos());
    ExecutionRequest request;
    request.requestId = "REQ_004";
    request.order = std::move(testOrder);
    
    std::vector<MarketData> marketData = {
        {"LowImpact", 2990.0, 3010.0, 100.0, 80.0, utils::TimeUtils::getCurrentNanos(), 1000.0, 500.0, 0.002, 0.003}, // Low impact
        {"HighImpact", 2995.0, 3015.0, 50.0, 30.0, utils::TimeUtils::getCurrentNanos(), 800.0, 400.0, 0.005, 0.004}, // High impact
        {"MediumImpact", 2992.0, 3012.0, 60.0, 40.0, utils::TimeUtils::getCurrentNanos(), 600.0, 300.0, 0.0025, 0.0035} // Medium impact
    };
    ...
    std::cout << "✓ MarketImpactStrategy created " << results.size() << " orders" << std::endl;
}
...
void testOrderRouterBasicFunctionality() {
    ...
    Order testOrder("ORDER_001", "BTC-USD", OrderSide::BUY, OrderType::LIMIT, 50000.0, 2.0, utils::TimeUtils::getCurrentNanos());
    ExecutionRequest request;
...
void testOrderRouterMultipleStrategies() {
    ...
    MarketData fastVenue = {"FastVenue", 50000.0, 50100.0, 5.0, 4.0, utils::TimeUtils::getCurrentNanos(), 500.0, 250.0, 0.001, 0.005};
    MarketData cheapVenue = {"CheapVenue", 50010.0, 50090.0, 8.0, 6.0, utils::TimeUtils::getCurrentNanos(), 600.0, 300.0, 0.002, 0.001};
    MarketData liquidVenue = {"LiquidVenue", 50005.0, 50095.0, 20.0, 18.0, utils::TimeUtils::getCurrentNanos(), 1000.0, 800.0, 0.0015, 0.003};
    ...
    std::vector<std::string> strategies = {"BEST_PRICE", "TWAP", "VWAP", "MARKET_IMPACT"};
    
    for (const auto& strategy : strategies) {
        router.setRoutingStrategy(strategy);
        
        Order testOrder("ORDER_" + strategy, "BTC-USD", OrderSide::BUY, OrderType::LIMIT, 50000.0, 5.0, utils::TimeUtils::getCurrentNanos());
        ExecutionRequest request;
        ...
        std::cout << "  Submitted " << strategy << " order with ID: " << submittedId << std::endl;
    }
