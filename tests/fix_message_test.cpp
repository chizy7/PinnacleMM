...
public:
    std::string createLogonMessage(const std::string& senderCompId, const std::string& targetCompId) {
...
int main() {
    ...
    std::cout << "Logon Message Length: " << logonMsg.length() << " bytes" << std::endl;
    
    // Test 2: Market data request
    std::cout << "\n2. Testing Market Data Request:" << std::endl;
    std::string mdRequest = fixMsg.createMarketDataRequest("AAPL");
    std::cout << "Market Data Request Length: " << mdRequest.length() << " bytes" << std::endl;
    
    // Test 3: Parse a sample message
    std::cout << "\n3. Testing Message Parsing:" << std::endl;
    std::string sampleMsg = "8=FIX.4.2\x01" "35=D\x01" "55=AAPL\x01" "54=1\x01" "38=100\x01" "44=150.00\x01";
