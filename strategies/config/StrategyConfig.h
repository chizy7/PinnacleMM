...
struct StrategyConfig {
  ...
  // Quote parameters
  double baseSpreadBps = 10.0; // Base spread in basis points (valid: > 0,
                               // minSpreadBps <= baseSpreadBps <= maxSpreadBps)
  double minSpreadBps = 5.0;   // Minimum spread in basis points (valid: > 0,
                               // minSpreadBps <= maxSpreadBps)
  double maxSpreadBps = 50.0;  // Maximum spread in basis points (valid: > 0,
                               // minSpreadBps <= maxSpreadBps)
  double orderQuantity =
      0.01; // Base order quantity (valid: > 0, minOrderQuantity <=
            // orderQuantity <= maxOrderQuantity)
  double minOrderQuantity = 0.001; // Minimum order quantity (valid: > 0,
                                   // minOrderQuantity <= orderQuantity)
  double maxOrderQuantity = 1.0;   // Maximum order quantity (valid: > 0,
  ...
  pinnacle::utils::Factor inventorySkewFactor = pinnacle::utils::Factor(
      0.5); // How much to skew quotes based on inventory
  double priceLevelSpacing =
      0.1; // Spacing between price levels as percentage of base spread
  size_t maxLevels = 3; // Maximum number of price levels to quote

  // Order book parameters
  pinnacle::utils::Factor volumeDepthFactor =
      pinnacle::utils::Factor(0.5); // How much to consider order book depth
  pinnacle::utils::Factor imbalanceThreshold =
      pinnacle::utils::Factor(0.3); // Threshold for order book imbalance
  pinnacle::utils::Factor volumeWeightFactor =
      pinnacle::utils::Factor(0.5); // Weight for volume-based price adjustment
  ...
  uint64_t tradeMonitoringIntervalMs =
      50; // Trade monitoring interval in milliseconds

  // Position management
  bool autoHedgeEnabled = false; // Enable auto-hedging
  double hedgeThresholdPct =
      50.0; // Threshold for auto-hedging (% of max position)
  ...
  uint64_t marketStressCheckMs =
      1000; // Market stress check interval in milliseconds
