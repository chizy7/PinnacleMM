...
void BasicMarketMaker::onOrderBookUpdate(const OrderBook& orderBook) {
    ...
    event.data = std::shared_ptr<void>(std::shared_ptr<OrderBook>{}, const_cast<OrderBook*>(&orderBook));
...
void BasicMarketMaker::onTrade(const std::string& symbol, double price, double quantity, 
...
void BasicMarketMaker::onOrderUpdate(const std::string& orderId, OrderStatus status, 
...
std::string BasicMarketMaker::getStatistics() const {
    ...
    oss << "  Position: " << std::fixed << std::setprecision(6) << m_position.load() << std::endl;
    oss << "  PnL: $" << std::fixed << std::setprecision(2) << m_pnl.load() << std::endl;
    ...
    oss << "  Total Volume Traded: " << std::fixed << std::setprecision(6) << m_stats.totalVolumeTraded << std::endl;
    oss << "  Max Position: " << std::fixed << std::setprecision(6) << m_stats.maxPosition << std::endl;
    oss << "  Min Position: " << std::fixed << std::setprecision(6) << m_stats.minPosition << std::endl;
    oss << "  Max PnL: $" << std::fixed << std::setprecision(2) << m_stats.maxPnL << std::endl;
    oss << "  Min PnL: $" << std::fixed << std::setprecision(2) << m_stats.minPnL << std::endl;
...
void BasicMarketMaker::strategyMainLoop() {
    ...
    while (!m_shouldStop.load(std::memory_order_acquire)) {
        ...
        if (currentTime - lastQuoteUpdateTime > m_config.quoteUpdateIntervalMs * 1000000) {
            updateQuotes();
        ...
        // Wait for events or timeout
        {
            ...
            m_eventCondition.wait_for(lock, 
                                    std::chrono::milliseconds(m_config.quoteUpdateIntervalMs / 2),
        ...
        switch (event.type) {
            ...
            case EventType::ORDER_UPDATE: {
                ...
                if (it != m_activeOrders.end()) {
                    ...
                    double fillDelta = updateInfo->filledQuantity - orderInfo.filledQuantity;
                    ...
                    if (fillDelta > 0) {
                        // Update position
                        double positionDelta = (orderInfo.side == OrderSide::BUY) ? fillDelta : -fillDelta;
...
double BasicMarketMaker::calculateTargetSpread() const {
    ...
    double targetSpread = m_config.baseSpreadBps * 0.0001 * m_orderBook->getMidPrice();
    ...
    double minSpread = m_config.minSpreadBps * 0.0001 * m_orderBook->getMidPrice();
    double maxSpread = m_config.maxSpreadBps * 0.0001 * m_orderBook->getMidPrice();
...
double BasicMarketMaker::calculateOrderQuantity(OrderSide side) const {
    ...
    return std::max(m_config.minOrderQuantity, std::min(quantity, m_config.maxOrderQuantity));
}
