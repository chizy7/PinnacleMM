...
public:
    ...
    bool initialize(const std::string& configPath, const std::string& masterPassword);
    bool isInitialized() const { return m_isInitialized; }
    
    std::shared_ptr<MarketDataFeed> getMarketDataFeed(const std::string& exchangeName);
...
private:
    WebSocketMarketDataFeed::Exchange getExchangeEnum(const std::string& exchangeName) const;
    ...
    std::unordered_map<std::string, std::shared_ptr<MarketDataFeed>> m_marketDataFeeds;
    std::mutex m_dataFeedsMutex;
    std::vector<std::string> m_supportedExchanges = {"coinbase", "kraken", "gemini", "binance", "bitstamp"};
};
