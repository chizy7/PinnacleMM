...
ExchangeConnectorFactory::getMarketDataFeed(const std::string &exchangeName) {
  ...
  spdlog::info("Creating WebSocket market data feed for exchange: {}",
               exchangeName);
  WebSocketMarketDataFeed::Exchange exchangeEnum =
      getExchangeEnum(exchangeName);
  ...
  auto webSocketFeed =
      WebSocketMarketDataFeed::create(exchangeEnum, m_apiCredentials);
  auto marketDataFeed =
      std::dynamic_pointer_cast<MarketDataFeed>(webSocketFeed);
...
ExchangeConnectorFactory::getSupportedExchanges() const {
...
bool ExchangeConnectorFactory::setApiCredentials(
  ...
  // Set credentials
  if (!m_apiCredentials->setCredentials(exchangeName, apiKey, apiSecret,
                                        passphrase)) {
    std::cerr << "Failed to set API credentials for " << exchangeName
              << std::endl;
    return false;
  }
