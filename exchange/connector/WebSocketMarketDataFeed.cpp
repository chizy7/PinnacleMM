...
// Create Coinbase JWT
std::string createCoinbaseJWT(const std::string& apiKey, const std::string& apiSecret) {
...
WebSocketMarketDataFeed::WebSocketMarketDataFeed(
    ...
    m_ssl_context = std::make_shared<boost::asio::ssl::context>(boost::asio::ssl::context::tlsv12_client);
...
std::shared_ptr<WebSocketMarketDataFeed> WebSocketMarketDataFeed::create(
    ...
    return std::shared_ptr<WebSocketMarketDataFeed>(new WebSocketMarketDataFeed(exchange, credentials));
}
...
void WebSocketMarketDataFeed::connectWebSocket() {
    try {
        ...
        auto ssl_stream = std::make_unique<boost::asio::ssl::stream<tcp::socket>>(*m_io_context, *m_ssl_context);
        ...
        throw std::runtime_error("WebSocket connection failed: " + std::string(e.what()));
    }
}
...
void WebSocketMarketDataFeed::onMessage(const std::string& message) {
    try {
        spdlog::debug("Received message: {}", message.substr(0, 200) + (message.length() > 200 ? "..." : ""));
...
bool WebSocketMarketDataFeed::sendSubscriptionInternal(const std::string& symbol) {
    try {
        std::string subscriptionMessage = createSubscriptionMessage(symbol);
        spdlog::info("Sending subscription for {}: {}", symbol, subscriptionMessage);
...
bool WebSocketMarketDataFeed::subscribeToMarketUpdates(const std::string& symbol, 
...
bool WebSocketMarketDataFeed::subscribeToOrderBookUpdates(const std::string& symbol,
