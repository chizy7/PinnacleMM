...
ExchangeSimulator::ExchangeSimulator(std::shared_ptr<OrderBook> orderBook)
    ...
    addMarketParticipant("taker", 10.0, 0.3);  // 10 trades per minute, 30% of volume
    addMarketParticipant("maker", 20.0, 0.4);  // 20 trades per minute, 40% of volume
    addMarketParticipant("noise", 5.0, 0.1);   // 5 trades per minute, 10% of volume
    addMarketParticipant("arbitrageur", 2.0, 0.2); // 2 trades per minute, 20% of volume
}
...
void ExchangeSimulator::setMarketDataFeed(std::shared_ptr<MarketDataFeed> marketDataFeed) {
...
void ExchangeSimulator::addMarketParticipant(const std::string& type, double frequency, double volumeRatio) {
    ...
    // Configure activity distribution (exponential with mean = 1/frequency minutes)
    participant.activityDistribution = std::exponential_distribution<double>(frequency / 60.0);
    
    // Set initial activity time
    participant.nextActivityTime = utils::TimeUtils::getCurrentSeconds() + 
        ...
        for (auto& participant : m_participants) {
            ...
            if (currentTime >= participant.nextActivityTime) {
                ...
                if (participant.type == "taker") {
                    ...
                    double quantity = 0.01 + (0.1 * participant.volumeRatio * (m_rng() % 10) / 10.0);
                    OrderSide side = (m_rng() % 2 == 0) ? OrderSide::BUY : OrderSide::SELL;
                    simulateMarketOrder(side, quantity);
                } else if (participant.type == "maker") {
                    // Makers submit limit orders
                    double quantity = 0.01 + (0.2 * participant.volumeRatio * (m_rng() % 10) / 10.0);
                    OrderSide side = (m_rng() % 2 == 0) ? OrderSide::BUY : OrderSide::SELL;
                    ...
                    double imbalance = m_orderBook ? m_orderBook->calculateOrderBookImbalance(5) : 0.0;
                    
                    if (std::abs(imbalance) > 0.3) {
                        // Trade against the imbalance
                        OrderSide side = (imbalance > 0) ? OrderSide::SELL : OrderSide::BUY;
                        double quantity = 0.05 + (0.2 * participant.volumeRatio * (m_rng() % 10) / 10.0);
                        simulateMarketOrder(side, quantity);
                    }
                ...
                participant.nextActivityTime = currentTime + 
                                           static_cast<uint64_t>(participant.activityDistribution(m_rng));
            }
        }
...
void ExchangeSimulator::simulateMarketOrder(OrderSide side, double quantity) {
    ...
    double executedQuantity = m_orderBook->executeMarketOrder(side, quantity, fills);
...
void ExchangeSimulator::simulateLimitOrder(OrderSide side, double price, double quantity) {
    if (!m_orderBook) {
        return;
    }
...
std::string ExchangeSimulator::generateOrderId() {
    ...
    oss << "sim-" << utils::TimeUtils::getCurrentNanos() << "-" << (m_rng() % 1000);
    return oss.str();
}
