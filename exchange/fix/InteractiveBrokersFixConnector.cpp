...
InteractiveBrokersFixConnector::InteractiveBrokersFixConnector(std::shared_ptr<utils::ApiCredentials> credentials)
...
bool InteractiveBrokersFixConnector::sendNewOrderSingle(const pinnacle::Order& order) {
    if (!isLoggedOn()) {
...
bool InteractiveBrokersFixConnector::replaceOrder(const std::string& orderId, const pinnacle::Order& newOrder) {
    if (!isLoggedOn()) {
...
void InteractiveBrokersFixConnector::onLogon() {
    std::cout << "IB FIX: Successfully logged on to Interactive Brokers" << std::endl;
...
void InteractiveBrokersFixConnector::onMarketDataMessage(const hffix::message_reader& msg) {
...
void InteractiveBrokersFixConnector::onExecutionReport(const hffix::message_reader& msg) {
    handleExecutionReport(msg);
}

void InteractiveBrokersFixConnector::onOrderCancelReject(const hffix::message_reader& msg) {
...
void InteractiveBrokersFixConnector::parseMarketDataSnapshot(const hffix::message_reader& msg) {
    ...
    std::string symbolStr = convertIBSymbolToInternal(std::string(symbol.begin(), symbol.end()));
    ...
    for (int i = 0; i < numEntries; ++i) {
        if (msg.find_with_hint(hffix::tag::MDEntryType, entryType, entryTypeIter)) {
            ...
            if (hffix::extract_char(entryType, type)) {
                ...
                if (msg.find_with_hint(hffix::tag::MDEntrySize, entrySize, entrySizeIter)) {
                    double size;
...
void InteractiveBrokersFixConnector::parseMarketDataIncrementalRefresh(const hffix::message_reader& msg) {
    // Similar to snapshot parsing but for incremental updates
    parseMarketDataSnapshot(msg); // Simplified implementation
}

std::string InteractiveBrokersFixConnector::convertIBSymbolToInternal(const std::string& ibSymbol) {
    ...
    internal.erase(std::find(internal.begin(), internal.end(), '\0'), internal.end());
    return internal;
}

std::string InteractiveBrokersFixConnector::convertInternalSymbolToIB(const std::string& internalSymbol) {
    ...
    if (internalSymbol.length() == 6 && internalSymbol.find("USD") != std::string::npos) {
        // Currency pair
...
void InteractiveBrokersFixConnector::handleExecutionReport(const hffix::message_reader& msg) {
    hffix::field clOrdId, symbol, side, orderQty, price, cumQty, avgPx, ordStatus, execType;
    
    std::string orderIdStr, symbolStr, sideStr, statusStr, execTypeStr;
    double orderQuantity = 0.0, orderPrice = 0.0, cumQuantity = 0.0, avgPrice = 0.0;
    ...
    if (msg.find_with_hint(hffix::tag::Symbol, symbol)) {
        symbolStr = convertIBSymbolToInternal(std::string(symbol.begin(), symbol.end()));
    }
    ...
    std::cout << "IB FIX: Execution Report - Order: " << orderIdStr 
              ...
              << ", Side: " << sideStr
              << ", Status: " << statusStr 
              << ", Exec Type: " << execTypeStr
              << ", Cum Qty: " << cumQuantity 
              << ", Avg Px: " << avgPrice << std::endl;
...
void InteractiveBrokersFixConnector::subscribeToConfiguredSymbols() {
    ...
    for (const auto& symbol : m_pendingSubscriptions) {
        ...
        std::cout << "IB FIX: Subscribed to market data for " << symbol << std::endl;
    }
