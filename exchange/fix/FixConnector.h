...
public:
     ...
     */
    FixConnector(const FixConfig& config, std::shared_ptr<utils::ApiCredentials> credentials);
    ...
    virtual bool replaceOrder(const std::string& orderId, const pinnacle::Order& newOrder) = 0;
...
protected:
     ...
     */
    hffix::message_writer createMarketDataRequest(const std::string& symbol, char subscriptionRequestType);
