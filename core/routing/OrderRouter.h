...
struct ExecutionRequest {
    ...
    std::string routingStrategy{"BEST_PRICE"}; // BEST_PRICE, TWAP, VWAP, MARKET_IMPACT
...
public:
     ...
     */
    virtual std::vector<ExecutionRequest> planExecution(
...
public:
    std::vector<ExecutionRequest> planExecution(
        const ExecutionRequest& originalRequest,
...
public:
    explicit TWAPStrategy(int numSlices = 10, std::chrono::seconds sliceInterval = std::chrono::seconds(30))
...
public:
     ...
     */
    bool addVenue(const std::string& venueName, const std::string& connectionType = "websocket");
...
private:
     ...
     */
    std::unordered_map<std::string, std::unique_ptr<RoutingStrategy>> m_strategies;
    ...
    struct Statistics {
        ...
        double bestFillRate{0.0}; // percentage of orders getting best available price
    };
    ...
    double calculateMarketImpact(const ExecutionRequest& request, const MarketData& venue);
    double calculateTotalCost(const ExecutionRequest& request, const MarketData& venue);
