...
std::vector<ExecutionRequest> BestPriceStrategy::planExecution(
    const ExecutionRequest& originalRequest,
    ...
    double bestPrice = (originalRequest.order.getSide() == pinnacle::OrderSide::BUY) 
                      ? std::numeric_limits<double>::max() 
                      : 0.0;
    
    for (const auto& venue : marketData) {
        double price = (originalRequest.order.getSide() == pinnacle::OrderSide::BUY) 
                      ? venue.askPrice : venue.bidPrice;
        double totalCost = price + venue.fees;
        
        bool isBetter = (originalRequest.order.getSide() == pinnacle::OrderSide::BUY) 
                       ? (totalCost < bestPrice) : (totalCost > bestPrice);
    ...
    request.order = Order(
        originalRequest.order.getOrderId(),
        originalRequest.order.getSymbol(),
        originalRequest.order.getSide(),
        originalRequest.order.getType(),
        originalRequest.order.getPrice(),
        originalRequest.order.getQuantity(),
...
std::vector<ExecutionRequest> TWAPStrategy::planExecution(
    const ExecutionRequest& originalRequest,
    const std::vector<MarketData>& marketData) {
    ...
    for (int i = 0; i < m_numSlices; ++i) {
        ...
        sliceRequest.requestId = originalRequest.requestId + "_SLICE_" + std::to_string(i);
...
std::vector<ExecutionRequest> VWAPStrategy::planExecution(
    const ExecutionRequest& originalRequest,
    const std::vector<MarketData>& marketData) {
    ...
    if (totalMarketVolume <= 0) {
        ...
        return TWAPStrategy(10, std::chrono::seconds(60)).planExecution(originalRequest, marketData);
    }
    ...
    if (totalQuantity > maxParticipation) {
        ...
        for (const auto& venue : marketData) {
            ...
            if (venueQuantity < 1.0) continue;
            
            ExecutionRequest venueRequest;
            venueRequest.requestId = originalRequest.requestId + "_VWAP_" + venue.venue;
            venueRequest.targetVenue = venue.venue;
            venueRequest.maxExecutionTime = originalRequest.maxExecutionTime;
            venueRequest.maxSlippage = originalRequest.maxSlippage;
            venueRequest.allowPartialFills = originalRequest.allowPartialFills;
...
std::vector<ExecutionRequest> MarketImpactStrategy::planExecution(
    const ExecutionRequest& originalRequest,
    const std::vector<MarketData>& marketData) {
    ...
    for (const auto& venue : sortedVenues) {
        ...
        if (venue.impactCost > m_maxImpactThreshold) break;
        
        // Calculate max quantity for this venue without exceeding impact threshold
        double availableSize = (originalRequest.order.getSide() == pinnacle::OrderSide::BUY) 
                              ? venue.askSize : venue.bidSize;
        
        // Use conservative fraction of available liquidity
        double maxVenueQty = std::min(availableSize * 0.3, remainingQuantity);
        
        if (maxVenueQty < 1.0) continue;
        
        ExecutionRequest venueRequest;
        venueRequest.requestId = originalRequest.requestId + "_IMPACT_" + venue.venue;
        venueRequest.targetVenue = venue.venue;
        venueRequest.maxExecutionTime = originalRequest.maxExecutionTime;
        venueRequest.maxSlippage = originalRequest.maxSlippage;
        venueRequest.allowPartialFills = originalRequest.allowPartialFills;
...
std::string OrderRouter::submitOrder(const ExecutionRequest& request) {
    ...
    modifiedRequest.order = Order(
        request.order.getOrderId(),
        request.order.getSymbol(),
        request.order.getSide(),
        request.order.getType(),
        request.order.getPrice(),
        request.order.getQuantity(),
    ...
    if (!m_executionQueue.tryEnqueue(std::move(modifiedRequest))) {
        std::cerr << "Failed to queue execution request: " << requestId << std::endl;
        return "";
    }
...
void OrderRouter::setExecutionCallback(std::function<void(const ExecutionResult&)> callback) {
    std::lock_guard<std::mutex> lock(m_callbackMutex);
    m_executionCallback = callback;
}

bool OrderRouter::addVenue(const std::string& venueName, const std::string& connectionType) {
    ...
    std::cout << "Added venue: " << venueName << " (type: " << connectionType << ")" << std::endl;
    return true;
}

void OrderRouter::updateMarketData(const std::string& venue, const MarketData& data) {
    std::lock_guard<std::mutex> lock(m_venuesMutex);
...
std::string OrderRouter::getStatistics() const {
    ...
    {
        std::lock_guard<std::mutex> venueLock(const_cast<std::mutex&>(m_venuesMutex));
...
void OrderRouter::routingThreadLoop() {
    while (!m_shouldStop.load()) {
        ...
        while (m_executionQueue.tryDequeue(request)) {
            ...
            std::vector<MarketData> marketData = getAllMarketData(request.order.getSymbol());
            
            // Apply routing strategy
            auto strategy = m_strategies.find(m_currentStrategy);
            if (strategy == m_strategies.end()) {
                std::cerr << "Invalid routing strategy: " << m_currentStrategy << std::endl;
                continue;
            }
        ...
        while (m_cancelQueue.tryDequeue(cancelRequestId)) {
            ...
            if (it != m_activeExecutions.end()) {
                // Mark as canceled - actual cancellation would happen in execution thread
...
std::vector<MarketData> OrderRouter::getAllMarketData(const std::string& /* symbol */) {
    std::lock_guard<std::mutex> lock(m_venuesMutex);
    ...
    for (const auto& venue : m_venues) {
        if (!venue.second.isActive) continue;
...
void OrderRouter::executeOrder(const ExecutionRequest& request) {
    // Simulate order execution - in reality this would connect to actual exchanges
...
void OrderRouter::updateExecutionResult(const ExecutionResult& result) {
    ...
    // Update statistics
    {
        ...
        double totalTime = m_stats.avgExecutionTime * (m_stats.completedExecutions - 1);
        m_stats.avgExecutionTime = (totalTime + result.executionTime / 1000.0) / m_stats.completedExecutions;
    }
}
