...
public:
    ...
    struct PinConfig {
        std::string hostname;
        std::vector<std::string> sha256Pins; // Base64-encoded SHA256 hashes of public keys
        bool enforcePin;
     ...
     */
    void addPin(const std::string& hostname, const std::string& sha256Pin, bool enforce = true);
...
private:
     ...
     */
    bool extractPublicKeyHash(X509* cert, unsigned char hash[SHA256_DIGEST_LENGTH]);
     ...
     */
    size_t base64ToHash(const std::string& base64, unsigned char* hash, size_t maxLength);
};
