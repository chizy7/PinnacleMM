...
SlidingWindowRateLimiter::SlidingWindowRateLimiter(const RateLimitConfig& config)
    : m_config(config) {}
...
RateLimitResult SlidingWindowRateLimiter::checkRequest(const std::string& key) {
    ...
    if (m_config.cooldownPeriod.count() > 0) {
        window.cooldownUntil = std::chrono::steady_clock::now() + m_config.cooldownPeriod;
    }
    ...
    return RateLimitResult(false, 0, m_config.cooldownPeriod, "Rate limit exceeded");
}
...
void RateLimiter::addRateLimit(const std::string& category, const RateLimitConfig& config) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
RateLimitResult RateLimiter::checkRequest(const std::string& category, const std::string& key) {
    if (!m_enabled) {
        return RateLimitResult(true, SIZE_MAX, std::chrono::milliseconds(0), "Rate limiting disabled");
    }
    ...
    if (it == m_limiters.end()) {
        ...
        return RateLimitResult(true, SIZE_MAX, std::chrono::milliseconds(0), "No rate limit configured");
    }
    
    return it->second->checkRequest(key);
}

void RateLimiter::recordRequest(const std::string& category, const std::string& key) {
    if (!m_enabled) {
        return;
    }
...
RateLimitResult RateLimiter::checkAndRecord(const std::string& category, const std::string& key) {
    if (!m_enabled) {
        return RateLimitResult(true, SIZE_MAX, std::chrono::milliseconds(0), "Rate limiting disabled");
    }
...
void RateLimiter::initializeDefaults() {
    // API rate limits
    addRateLimit("api_general", RateLimitConfig(1000, std::chrono::minutes(1), std::chrono::seconds(60)));
    addRateLimit("api_market_data", RateLimitConfig(5000, std::chrono::minutes(1), std::chrono::seconds(30)));
    
    // Authentication rate limits
    addRateLimit("auth_login", RateLimitConfig(5, std::chrono::minutes(15), std::chrono::minutes(5)));
    addRateLimit("auth_password_reset", RateLimitConfig(3, std::chrono::hours(1), std::chrono::minutes(15)));
    
    // Trading operation rate limits
    addRateLimit("orders_submit", RateLimitConfig(100, std::chrono::minutes(1), std::chrono::seconds(30)));
    addRateLimit("orders_cancel", RateLimitConfig(500, std::chrono::minutes(1), std::chrono::seconds(15)));
    addRateLimit("orders_modify", RateLimitConfig(200, std::chrono::minutes(1), std::chrono::seconds(30)));
    
    // Configuration access rate limits
    addRateLimit("config_read", RateLimitConfig(100, std::chrono::minutes(1)));
    addRateLimit("config_write", RateLimitConfig(10, std::chrono::minutes(5), std::chrono::minutes(1)));
    
    // Network connection rate limits
    addRateLimit("websocket_connections", RateLimitConfig(50, std::chrono::minutes(5), std::chrono::minutes(2)));
    addRateLimit("fix_connections", RateLimitConfig(10, std::chrono::minutes(5), std::chrono::minutes(5)));
}

// RateLimitGuard implementation
RateLimitGuard::RateLimitGuard(const std::string& category, const std::string& key)
