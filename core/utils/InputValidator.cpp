...
const std::vector<std::regex> InputValidator::XSS_PATTERNS = {
    std::regex(R"(<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>)",
               std::regex_constants::icase),
...
InputValidator::validateString(const std::string &input, size_t minLength,
  ...
  if (input.length() > maxLength) {
    return ValidationResult(false, "String too long (maximum " +
...
ValidationResult InputValidator::validateSymbol(const std::string &symbol) {
  ...
  for (char c : symbol) {
    if (!isValidSymbolChar(c)) {
      return ValidationResult(false, "Symbol contains invalid character: '" +
                                         std::string(1, c) + "'");
    }
  }
...
ValidationResult InputValidator::validateOrderId(const std::string &orderId) {
  ...
  for (char c : orderId) {
    if (!std::isalnum(c) && c != '-' && c != '_') {
      return ValidationResult(false, "Order ID contains invalid character: '" +
                                         std::string(1, c) + "'");
    }
  }
...
ValidationResult InputValidator::validateConfigPath(const std::string &path) {
  ...
  if (!hasExtension(".json") && !hasExtension(".conf") &&
      !hasExtension(".config")) {
    return ValidationResult(
        false, "Config file must have .json, .conf, or .config extension");
  }
...
ValidationResult InputValidator::validatePassword(const std::string &password) {
  ...
  if (!(hasUpper && hasLower && hasDigit && hasSpecial)) {
    return ValidationResult(false, "Password must contain uppercase, "
  ...
  for (const auto &pattern : SQL_INJECTION_PATTERNS) {
    if (std::regex_search(input, pattern)) {
      spdlog::warn("Potential SQL injection attempt detected: {}",
                   input.substr(0, 50));
      return ValidationResult(
          false, "Input contains potentially malicious SQL patterns");
    }
  }
...
ValidationResult InputValidator::checkForXss(const std::string &input) {
  for (const auto &pattern : XSS_PATTERNS) {
    if (std::regex_search(input, pattern)) {
      ...
      return ValidationResult(
          false, "Input contains potentially malicious script patterns");
    }
  }
...
InputValidator::checkForPathTraversal(const std::string &path) {
  for (const auto &pattern : PATH_TRAVERSAL_PATTERNS) {
    if (std::regex_search(path, pattern)) {
      ...
      return ValidationResult(false,
                              "Path contains potential traversal patterns");
    }
  }
...
std::unordered_set<std::string> InputValidator::getSupportedExchanges() {
  return {"coinbase", "kraken", "binance",
          "bitstamp", "gemini", "interactive_brokers",
          "oanda",    "fxcm",   "simulator"};
}
