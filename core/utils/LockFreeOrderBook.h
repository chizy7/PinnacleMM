...
public:
    ...
    // Get total quantity
    double getTotalQuantity() const { return m_totalQuantity.load(std::memory_order_acquire); }
    
    // Get order count
    size_t getOrderCount() const { return m_orderCount.load(std::memory_order_acquire); }
    ...
    void forEachOrder(const std::function<void(std::shared_ptr<Order>)>& func) const;
};
...
public:
    ...
    std::optional<std::pair<double, std::shared_ptr<LockFreePriceLevel>>> getBestLevel() const;
    
    // Get all price levels up to a certain depth
    std::vector<std::shared_ptr<LockFreePriceLevel>> getLevels(size_t depth) const;
    ...
    void forEachLevel(const std::function<void(double, std::shared_ptr<LockFreePriceLevel>)>& func) const;
};
...
template<typename Comparator>
LockFreePriceMap<Comparator>::WriteGuard::WriteGuard(std::atomic_flag& lock) : m_lock(lock) {
    while (m_lock.test_and_set(std::memory_order_acquire)) {
        // Spin until we acquire the lock
        // Use a busy-wait instead of std::this_thread::yield() for broader compatibility
    }
}
...
template<typename Comparator>
bool LockFreePriceMap<Comparator>::insertLevel(double price, std::shared_ptr<LockFreePriceLevel> level) {
...
template<typename Comparator>
std::shared_ptr<LockFreePriceLevel> LockFreePriceMap<Comparator>::findLevel(double price) const {
    PriceMap* current = m_current.load(std::memory_order_acquire);
...
template<typename Comparator>
std::optional<std::pair<double, std::shared_ptr<LockFreePriceLevel>>> LockFreePriceMap<Comparator>::getBestLevel() const {
    PriceMap* current = m_current.load(std::memory_order_acquire);
    ReadGuard guard(*current);
...
template<typename Comparator>
std::vector<std::shared_ptr<LockFreePriceLevel>> LockFreePriceMap<Comparator>::getLevels(size_t depth) const {
    PriceMap* current = m_current.load(std::memory_order_acquire);
    ReadGuard guard(*current);
...
template<typename Comparator>
void LockFreePriceMap<Comparator>::clear() {
    WriteGuard guard(m_writeLock);
...
void LockFreePriceMap<Comparator>::forEachLevel(
    const std::function<void(double, std::shared_ptr<LockFreePriceLevel>)>& func) const {
...
public:
    ...
    // Order book depth
    std::vector<std::shared_ptr<LockFreePriceLevel>> getBidLevels(size_t depth) const;
    std::vector<std::shared_ptr<LockFreePriceLevel>> getAskLevels(size_t depth) const;
