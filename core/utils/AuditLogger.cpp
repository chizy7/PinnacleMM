...
bool AuditLogger::initialize(const std::string &logPath, size_t maxFileSize,
  ...
  try {
    ...
    m_logger =
        spdlog::rotating_logger_mt("audit", logPath, maxFileSize, maxFiles);
    ...
    // Log initialization
    AuditEvent initEvent(AuditEventType::SYSTEM_START,
                         "Audit logging initialized", true);
    logEvent(initEvent);
...
void AuditLogger::setCurrentSession(const std::string &userId,
  ...
  AuditEvent event(AuditEventType::AUTHENTICATION_SUCCESS,
...
std::string AuditLogger::eventTypeToString(AuditEventType type) {
  ...
  case AuditEventType::AUTHENTICATION_FAILURE:
    return "AUTH_FAILURE";
  case AuditEventType::CREDENTIAL_ACCESS:
    return "CREDENTIAL_ACCESS";
  case AuditEventType::CREDENTIAL_MODIFICATION:
    return "CREDENTIAL_MODIFICATION";
  case AuditEventType::CONFIG_ACCESS:
    return "CONFIG_ACCESS";
  case AuditEventType::CONFIG_MODIFICATION:
    return "CONFIG_MODIFICATION";
  case AuditEventType::NETWORK_CONNECTION:
    return "NETWORK_CONNECTION";
  case AuditEventType::NETWORK_FAILURE:
    return "NETWORK_FAILURE";
  case AuditEventType::ORDER_SUBMISSION:
    return "ORDER_SUBMISSION";
  case AuditEventType::ORDER_MODIFICATION:
    return "ORDER_MODIFICATION";
  case AuditEventType::ORDER_CANCELLATION:
  ...
  case AuditEventType::SYSTEM_STOP:
    return "SYSTEM_STOP";
  case AuditEventType::ERROR_CONDITION:
...
std::string AuditLogger::formatEventAsJson(const AuditEvent &event) {
  ...
  json["session_id"] =
      event.sessionId.empty() ? m_currentSessionId : event.sessionId;
