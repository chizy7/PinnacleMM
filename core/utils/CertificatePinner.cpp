...
void CertificatePinner::addPin(const std::string& hostname, const std::string& sha256Pin, bool enforce) {
...
bool CertificatePinner::verifyCertificate(const std::string& hostname, X509* cert) {
    if (!m_enabled || !cert) {
    ...
    if (pinConfig.enforcePin) {
        spdlog::error("Certificate pin verification failed for {}. Expected one of {} pins, got: {}", 
                     hostname, pinConfig.sha256Pins.size(), certFingerprint.substr(0, 20) + "...");
        return false;
    } else {
...
bool CertificatePinner::loadPinsFromFile(const std::string& configPath) {
    try {
        ...
        if (!file.is_open()) {
            spdlog::warn("Certificate pin configuration file not found: {}", configPath);
            return false;
        }
        ...
        if (!config.contains("certificate_pins") || !config["certificate_pins"].is_object()) {
...
bool CertificatePinner::savePinsToFile(const std::string& configPath) {
    try {
        ...
        if (!file.is_open()) {
            spdlog::error("Failed to open certificate pin config file for writing: {}", configPath);
            return false;
        }
...
void CertificatePinner::initializeDefaultPins() {
    // Coinbase Pro pins (note to self:just an example - these should be updated with real pins)
    addPin("ws-feed.exchange.coinbase.com", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=", false);
    
    // Kraken pins
    addPin("ws.kraken.com", "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB=", false);
    
    // Binance pins  
    addPin("stream.binance.com", "CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC=", false);
    ...
    spdlog::info("Initialized default certificate pins (example pins - update for production)");
}

bool CertificatePinner::extractPublicKeyHash(X509* cert, unsigned char hash[SHA256_DIGEST_LENGTH]) {
...
std::string CertificatePinner::hashToBase64(const unsigned char* hash, size_t length) {
...
size_t CertificatePinner::base64ToHash(const std::string& base64, unsigned char* hash, size_t maxLength) {
