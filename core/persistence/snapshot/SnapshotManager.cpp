...
SnapshotManager::SnapshotManager(const std::string& snapshotDirectory, const std::string& symbol)
    ...
    if (!snapshots.empty()) {
        m_latestSnapshotId.store(*std::max_element(snapshots.begin(), snapshots.end()),
...
bool SnapshotManager::cleanupOldSnapshots(int keepCount) {
    ...
    for (size_t i = keepCount; i < snapshots.size(); ++i) {
        ...
        try {
            ...
            std::cerr << "Failed to delete snapshot " << path << ": " << e.what() << std::endl;
            success = false;
        }
    }
...
std::string SnapshotManager::getSnapshotPath(uint64_t snapshotId) const {
    ...
    oss << m_snapshotDirectory << "/" << m_symbol << "-" << snapshotId << ".snapshot";
    return oss.str();
}
...
std::vector<uint64_t> SnapshotManager::listSnapshots() const {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(m_snapshotDirectory)) {
        ...
        if (filename.find(prefix) == 0 && filename.find(suffix) == filename.length() - suffix.length()) {
            ...
            std::string idStr = filename.substr(prefix.length(), 
...
bool SnapshotManager::writeSnapshotToFile(uint64_t snapshotId, const OrderBook& orderBook) {
    // Get snapshot file path
    std::string path = getSnapshotPath(snapshotId);
    
    // For now, I'll use a simple serialization approach
    // In a full implementation, I'd use memory mapping and a more efficient format
    ...
    try {
        ...
        for (const auto& level : bids) {
            ...
            file.write(reinterpret_cast<const char*>(&level.totalQuantity), sizeof(level.totalQuantity));
            
            // Write order count
            size_t orderCount = level.orders.size();
            file.write(reinterpret_cast<const char*>(&orderCount), sizeof(orderCount));
            ...
            for (const auto& order : level.orders) {
                ...
                file.write(reinterpret_cast<const char*>(&orderIdLength), sizeof(orderIdLength));
                ...
                file.write(reinterpret_cast<const char*>(&filledQuantity), sizeof(filledQuantity));
                
                // Write order timestamp
                uint64_t orderTimestamp = order->getTimestamp();
                file.write(reinterpret_cast<const char*>(&orderTimestamp), sizeof(orderTimestamp));
            }
        }
...
std::shared_ptr<OrderBook> SnapshotManager::readSnapshotFromFile(uint64_t snapshotId) {
    // Get snapshot file path
    std::string path = getSnapshotPath(snapshotId);
    ...
    try {
        ...
        for (size_t i = 0; i < bidCount; ++i) {
            ...
            for (size_t j = 0; j < orderCount; ++j) {
                ...
                file.read(reinterpret_cast<char*>(&orderIdLength), sizeof(orderIdLength));
                ...
                file.read(reinterpret_cast<char*>(&filledQuantity), sizeof(filledQuantity));
                
                // Read order timestamp
                uint64_t orderTimestamp;
                file.read(reinterpret_cast<char*>(&orderTimestamp), sizeof(orderTimestamp));
