...
Order::Order(Order&& other) noexcept
    ...
    m_filledQuantity.store(other.m_filledQuantity.load(std::memory_order_relaxed), std::memory_order_relaxed);
    m_lastUpdateTime.store(other.m_lastUpdateTime.load(std::memory_order_relaxed), std::memory_order_relaxed);
}
...
bool Order::cancel(uint64_t timestamp) {
    ...
    if (currentStatus == OrderStatus::NEW || currentStatus == OrderStatus::PARTIALLY_FILLED) {
...
bool Order::isActive() const {
    OrderStatus currentStatus = m_status.load(std::memory_order_relaxed);
    return currentStatus == OrderStatus::NEW || currentStatus == OrderStatus::PARTIALLY_FILLED;
}
...
bool Order::operator<(const Order& other) const {
    // Primary ordering by price (buy: higher price has priority, sell: lower price has priority)
