...
LockFreeOrderBook::calculateVolumeWeightedAveragePrice(OrderSide side,
                                                       double quantity) const {
  return m_lockFreeOrderBook->calculateVolumeWeightedAveragePrice(side,
                                                                  quantity);
}
...
std::shared_ptr<OrderBookSnapshot> LockFreeOrderBook::getSnapshot() const {
  ...
  auto snapshot = std::make_shared<OrderBookSnapshot>(
      getSymbol(), utils::TimeUtils::getCurrentNanos(),
...
void LockFreeOrderBook::clear() { m_lockFreeOrderBook->clear(); }
