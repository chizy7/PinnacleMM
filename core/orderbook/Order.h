...
public:
    ...
    // Copy constructor and assignment operator (deleted to prevent unexpected copies) 
    ...
    OrderStatus getStatus() const { return m_status.load(std::memory_order_relaxed); }
    double getPrice() const { return m_price; }
    double getQuantity() const { return m_quantity; }
    double getFilledQuantity() const { return m_filledQuantity.load(std::memory_order_relaxed); }
    double getRemainingQuantity() const;
    uint64_t getTimestamp() const { return m_timestamp; }
    uint64_t getLastUpdateTime() const { return m_lastUpdateTime.load(std::memory_order_relaxed); }
...
private: 
    ...
    std::atomic<OrderStatus> m_status;  // Current status (atomic for thread safety)
    double m_price;                     // Price level
    double m_quantity;                  // Original quantity
    std::atomic<double> m_filledQuantity; // Filled quantity (atomic for thread safety)
